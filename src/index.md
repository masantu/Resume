...
使用 `Web.py` 搭建支持块存储/文件存储的管理系统（`IaaS`）。可实现管理磁盘/磁盘阵列功能以及 `LVM` 权限/配额管理功能。通过`Socket`连接实现控制器间通信，同时基于`Rsync` 实现两者之间文件同步，多线程、多进程提高`IO`效率。基于 `RBAC` 实现访问控制，`IPSAN` 和 `FCSAN` 可选协议构建灵活的 `SAN` 块存储网络，基于 `NFS` 和 `SAMBA` 搭建 `NAS` 文件存储服务。利用 `DRBD` 实现数据远端备份，`HA` 技术实现主备双活，提高系统可靠性及可用性。

**技术关键词**：Web.py + RAID + IPSAN + FCSAN + DRBD + wxPython

**项目模块**    

1.系统概览 2.网络设置 3.磁盘管理 4.`RAID`管理 5.`NAS` 管理 6.`SAN` 管理 7.系统工具 8.报警设置 9.日志管理 10.用户权限管理 11.远程镜像等；

#### 责任描述

使用 `DRBD` 开发实现数据容灾，支持主备热切换以及数据镜像备份与恢复；实现基于 `RBAC` 的权限管理模块的数据库设计、前端界面及后端逻辑代码编写及功能测试；日常 `bug` 修复及模块功能调优；新功能的需求分析及方案设计，帮助新组员理解业务功能以及对较难实现的部分进行功能分析及复杂问题拆分；技术文档的编写、整理及完善。
<fancybox>
...
后端使用 `Django` 框架为用户精准呈现实时房屋信息并实现酒店预订功能，后台为商户提供房源信息发布和订单管理功能。使用第三方接口实现商品图片信息存储以及图片验证码+短信验证码实现用户验证，`MySQL` 数据库存储并使用 `Redis` 缓存热点数据，使用 `Whoosh` 全文检索引擎实现商品搜索功能，服务端使用 `Nginx` 反向代理实现部署。
...
参与前期数据库结构的设计，并使用第三方接口实现用户短信验证码验证及用户信息登记，完成用户登录及个人信息管理;负责实现各详细分类下的具体商品按照查询条件排序展示、商品列表页分页以及 `Redis` 缓存实现数据存储优化，从而实现商品列表的快速展示，配合前端人员完成接口对接与协调开发。
...
def no_disturbance_if_you_do_not_mean_that(altchars):
    ret = eval(base64.b64decode(altchars))
    return ret


if __name__ == '__main__':
    hard_read_bytes = 'eydRUSc6ICc0NDU1Njc4ODknLCAncGhvbmUnOiAnMTgyMzY5NTY3ODYnLCAnZW1haWwnOiAnaW1veWFvQDEyNi5jb20nLCAnd2VjaGF0JzogJ2ltb3lh' \
                      'bycsICdnZW5kZXInOiAnbWFsZSd9'.encode('utf-8')
    print(no_disturbance_if_you_do_not_mean_that(hard_read_bytes))
```
